...
public class BowtieAlignmentCommand extends ProjectCommand {
	...
	protected List<Option> createOptions() {
		...
		toret.add(new DefaultValuedOption("bowtie-seedlen", "l", 
				"The \"seed length\"; i.e., the number of bases on the high-quality end of the read to which the -n ceiling applies. The lowest permitted setting is 5 and the default is 28. bowtie is faster for larger values of -l.", "20"));
		
		toret.add(new DefaultValuedOption("bowtie-seedmms", "n", 
				"Maximum number of mismatches permitted in the \"seed\", i.e. the first L base pairs of the read (where L is set with -l/--bowtie-seedlen). This may be 0, 1, 2 or 3", "0"));
		...
		/*toret.add(new Option("bowtie-fr", "fr", 
				"The upstream/downstream mate orientations for a valid paired-end alignment against the forward reference strand: forward-reverse (this is the default)", true, false));
		toret.add(new Option("bowtie-fr", "rf", 
				"The upstream/downstream mate orientations for a valid paired-end alignment against the forward reference strand: reverse-forward", true, false));
		toret.add(new Option("bowtie-fr", "ff", 
				"The upstream/downstream mate orientations for a valid paired-end alignment against the forward reference strand: forward-forward", true, false));
		*/
		...
		*/
		/*toret.add(new DefaultValuedOption("bowtie-m", "m", 
				"Suppress all alignments for a particular read or pair if more than <m> reportable alignments exist for it", "1"));
		*/
