...
public class Tools {
	...
	private HashMap<Thread, HashMap<Strand, ArrayList<SAMFileReader>>> perThreadReaders = new HashMap<Thread, HashMap<Strand, ArrayList<SAMFileReader>>>();
	
	
	public Collection<RecordAndOffset> getReadsForLocus(GenomeAnalysisEngine toolkit, Strand strand, String contig, int pos, boolean filterDuplicates){
	...
	public ReadBackedPileup getReadsForStrand(final Strand strand, final AlignmentContext alnContext, boolean filterDuplicates){
				...
				while (it.hasNext()){
					...
					if (strand.isNegative()){
						if (!uniqueReads.containsKey(element.getRead().getAlignmentEnd()) ||
								sumQuality(uniqueReads.get(element.getRead().getAlignmentEnd()).getRead()) < sumQuality(element.getRead())){
						...
						if (!uniqueReads.containsKey(element.getRead().getAlignmentStart()) ||
								sumQuality(uniqueReads.get(element.getRead().getAlignmentStart()).getRead()) < sumQuality(element.getRead())){
