...
class MethylationStatistics {
	...
	public String toString() {
		...
		for (Context c : Context.values()) {
			toret.append(" " + c + " called methylcytosines: " + mCCounts.get(c) + "/" + cCounts.get(c) + " (" +
					(double) mCCounts.get(c) / (double) cCounts.get(c) + ")\n");
		}
		toret.append("Methylation Levels:\n");
		for (Context c : Context.values()) {
			toret.append(" " + c + ": " + mcReadsCounts.get(c) + "/" + ctReadsCounts.get(c) + " (" + (double)
					mcReadsCounts.get(c) / (double) ctReadsCounts.get(c) + ")\n");
		}
