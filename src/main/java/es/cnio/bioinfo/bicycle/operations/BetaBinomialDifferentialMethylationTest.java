...
public class BetaBinomialDifferentialMethylationTest {
	...
	public double getPvalue(int[] treatmentCytosines, int[] treatmentDepth,
		...
		return getPvalueValidSamples(validTreatmentCytosines, validTreatmentDepth, validControlCytosines, validControlDepth);
	}
...
//		System.err.println("MLE muControl: "+muControl);
		...
		double likelihoodRatio = computeLikelihoodRatio(treatmentCytosines, treatmentDepth, controlCytosines, controlDepth, theta, mu, muTreatment, muControl);
...
//		System.err.println("Log likelihood ratio "+likelihoodRatio);
	...
	private double computePValue(double likelihoodRatio, int sampleSize) {
		...
		return new TDistribution(sampleSize).cumulativeProbability(-1.0 * Math.sqrt(Math.max(0.0d, likelihoodRatio))) * 2;
	}
	...
	private double computeLikelihoodRatio(int[] treatmentCytosines, int[] treatmentDepth, int[] controlCytosines,
			int[] controlDepth, double theta, double mu, double muTreatment, double muControl) {
	...
	private double logLikelihood(int[] cytosines, int[] depth, double mu, double theta) {
		...
		for (int j = 0; j < cytosines.length; j++) {
			logLikelihood += Gamma.logGamma(mu*theta + cytosines[j]) + Gamma.logGamma((1 - mu)*theta + depth[j] - cytosines[j]) +
					Gamma.logGamma(theta) - Gamma.logGamma(mu*theta) - Gamma.logGamma((1 - mu)*theta) - Gamma.logGamma(depth[j] + theta);
		}
...
//				System.err.println("muEst "+ muEst);
	...
	private double mleTheta(int[] treatmentCytosines, int[] treatmentDepth, int[] controlCytosines, int[] controlDepth) {
		final UnivariateFunction function = new UnivariateFunction() {
...
//		System.err.println("thetaDerivative("+interval[1]+") = "+function.value(interval[1]));
	...
	private double thetaDerivative(double theta, int[] treatmentCytosines, int[] treatmentDepth, int[] controlCytosines, int[] controlDepth) { 
//		System.err.println("computing derivative for theta "+theta);
		...
		for (int j = 0; j < treatmentCytosines.length; j++) {
			double currentValue = muTreatmentEst * Gamma.digamma(muTreatmentEst*theta + treatmentCytosines[j]) + 
					  (1 - muTreatmentEst) * Gamma.digamma((1 - muTreatmentEst)*theta + treatmentDepth[j] - treatmentCytosines[j]) + 
					  Gamma.digamma(theta) - 
...
//			System.err.println("current value "+j+" "+currentValue);
		...
		for (int j = 0; j < controlCytosines.length; j++) {
			double currentValue = muControlEst * Gamma.digamma(muControlEst*theta + controlCytosines[j]) + 
					(1 - muControlEst) * Gamma.digamma((1 - muControlEst)*theta + controlDepth[j] - controlCytosines[j]) + 
					Gamma.digamma(theta) - 
...
//		System.err.print("computing derivative for mu "+mu+", muest "+muEst+". ");
		for (int j = 0; j < cytosines.length; j++) {
			derivative += Gamma.digamma(muEst * thetaEst + cytosines[j]) - Gamma.digamma((1 - mu)*thetaEst + depth[j] - cytosines[j])
