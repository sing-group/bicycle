...
public class SampleBisulfitation {
		         ...
		         if(thisLine.startsWith("@")){
		        	 	...
		        	 	if(removeReadsWithUnconvertedBarCodes){
	        	 			...
	        	 			if (tokens.length==2){
		        	 			...
		        	 			if(thisReadBarcode.charAt(barcodePosition)!='t' && thisReadBarcode.charAt(barcodePosition)!='T') useRead=false;
		        	 ...
		        	 /*	if(removeUnconvertedReads){
		        	 	...
		        	 	if(useRead){//que no quiere comprobar si la read está mal bisulfitada y pasa de eliminarla, vamos por aquí
	...
	public File getBisulfitedFile(File file){
		...
		return new File(this.sample.getProject().getWorkingDirectory()+File.separator+"bisulfited_CT_"+getPlainFileName(file));		
	}
	private String getPlainFileName(File file){
		if (this.sample.getReadsFiles().size()==1 && this.sample.getName().equals(this.sample.getReadsFiles().get(0).getName())){
			//1-fastq sample
			return file.getName();
		}else{
	...
	public File getUnconvertedReads(File file){
		return new File(this.sample.getProject().getWorkingDirectory()+File.separator+"unconvertedReads_"+getPlainFileName(file));	
