...
public class ReferenceBisulfitation {
	...
	public File getBisulfitedReference(Replacement replacement, Reference ref) {
		...
		if (bisulfitedReferenceDirName != null) {
			return new File(bisulfitedReferenceDirName + File.separator + ref.getReferenceFile().getName() +
					"_bisulfited_" + replacement.name());
		} else {
			File tryOnReference = new File(this.project.getReferenceDirectory() + File.separator + ref
					.getReferenceFile().getName() + "_bisulfited_" + replacement.name());
			File tryOnWorking = new File(this.project.getWorkingDirectory() + File.separator + ref.getReferenceFile()
					.getName() + "_bisulfited_" + replacement.name());
			...
			throw new IllegalArgumentException("property not found in project " + BISULFITED_DIR_PROPERTY +
					replacement.name() + ". Did you perform reference bisulfitation?");

		}

	}

	public void computeReferenceBisulfitation(Replacement replacement, Reference reference, boolean onWorkingDir)
			throws IOException {
		...
		this.project.addProperty(BISULFITED_DIR_PROPERTY + replacement.name(), onWorkingDir ? project
				.getWorkingDirectory().getAbsolutePath() : project.getReferenceDirectory().getAbsolutePath());
